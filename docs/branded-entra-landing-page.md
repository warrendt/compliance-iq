...
1. Switch Easy Auth to **`AllowAnonymous`** (live app + IaC) so anonymous requests
   reach the Streamlit app instead of being pre-empted. The **backend** container
   app keeps the default forced-login redirect.
...
Switching to `AllowAnonymous` does **not** change token storage, scopes, or the
provider config — the token store stays enabled and the same scopes are still
requested at `/.auth/login/aad`. It only stops the forced pre-auth redirect.
...
| `app/frontend/utils/state_init.py` | Pre-existing bug fix (see below): added `recover_session_state()` + `clear_workflow_state()`. |
...
| `app/tests/test_frontend_landing.py`, `test_frontend_auth.py`, `test_workflow_state_recovery_frontend.py` | Tests for the gate decision, Easy-Auth detection, the header-only auth check, render output, and state recovery. |
...
recovery / deep-link handling, so anonymous users never trigger backend calls and
no sidebar/app chrome leaks behind the landing page (chrome is also hidden via CSS).
...
so the frontend failed to import at startup (`ImportError`) — `main` is broken
independently of this feature. Minimal, faithful wrappers over the existing restore
logic were added so the app imports and this feature can run. This overlaps with
in-flight work on `warrendt-improve-ai-mapping-accuracy`, which fixes the same gap
differently — reconcile on merge.
...
- Individual `pages/*.py` are not separately gated; the entrypoint (`app.py`) is.
  ...
  deep-linked pages degrade rather than expose data. Add `require_login()` to a page
...
- A separate known issue — the ARM token store returning empty at runtime — is
  **out of scope** here and does not affect the landing page or the sign-in
  redirect.
- End-to-end authenticated sign-in is validated structurally (the `/.auth/login/aad`
  302 to Entra, `AllowAnonymous`, and the enabled token store); a full interactive
  login round-trip needs real user credentials.
